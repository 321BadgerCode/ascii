...
string txt_to_ascii(string input,bool horizontal_reverse=false,bool vertical_reverse=false){
	string output = "";
	vector<vector<string>> chr(input.length(), vector<string>(6));
...
string txt_to_code(const string txt, const char start=' '){
	...
	return (string)blue+"string"+"\033[0m"+" "+(string)rgb(125,125,125)+"ascii_map"+"\033[0m["+(string)rgb(85,85,85)+to_string(i)+"\033[0m]={\n"+get_code(split_ascii(output),comments)+"};";
}
...
int main(int argc, char** argv){
	...
	for(int i=0;i<argc;i++){
		if(string(argv[i])=="-h"){
			cout<<txt_to_ascii(argv[0])<<endl<<endl;
			cout<<argv[0]<<" -t <text> -w <filename> -i <filename> -a <filename> -r <reverse>"<<endl;
			cout<<"-t <text> : text to convert to ascii art"<<endl;
			cout<<"-w <filename> : filename to write ascii art to"<<endl;
			cout<<"-i <filename> : filename to take as input text to be converted"<<endl;
			cout<<"-a <filename> : filename to convert ascii art to text"<<endl;
			cout<<"-r <reverse> : reverse the ascii art horizontally or vertically\n\t* h: horizontal\n\t* v: vertical\n\t* hv: horizontal & vertical";
			return 0;
		}
